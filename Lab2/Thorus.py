...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.gamma = 0.05
        self.acceleration = np.zeros(shape=(3, int(len(nodes_coords) / 3)), dtype=np.double)
        ...
        self.tetrs -= 1
        self.phi = 5 * np.pi/180

        self.matrixx = np.array([[1, 0, 0], [0, np.cos(self.phi), np.sin(self.phi)], [0, -np.sin(self.phi), np.cos(self.phi)]])
        self.matrixz = np.array([[np.cos(self.phi), np.sin(self.phi), 0], [-np.sin(self.phi), np.cos(self.phi), 0], [0, 0, 1]])
    # Метод отвечает за выполнение для всей сетки шага по времени величиной tau

    def move(self, tau):
        # По сути метод просто двигает все точки c их текущими скоростями
        self.nodes += self.velocity * tau
        r_3 = np.power(np.sqrt(np.sum(np.power(self.nodes, 2), axis=0)), 3)
        # print(self.nodes)
        # print(self.smth)
        for i in range(self.nodes.shape[1]):
            self.nodes[:, i] = self.matrixz.dot(self.nodes[:, i])
            self.smth[i] += np.sin(10*i/np.pi)
        self.acceleration = self.gamma * np.divide(self.nodes, r_3)
...
try:
    ...
    gmsh.merge(os.path.join(path, '/Users/arseniy/Desktop/Miniscience4Term/Lab1/Thorus.stl'))
except:
